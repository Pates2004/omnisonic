...
env\python.exe -m pip install wxPython sounddevice soundfile numpy
...
%PYTHON_CMD% -c "import json, sys, os; sys.exit(0 if (os.path.exists('settings.json') and json.load(open('settings.json', encoding='utf-8')).get('validate_components', False)) else 1)" 2>nul
IF %ERRORLEVEL% EQU 0 (
