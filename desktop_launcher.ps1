...
public static class OmniSonicConsoleWindow {
...
function Show-LauncherConsole { Set-LauncherConsoleVisible $true }
...
function Get-HideConsolePreference {
    ...
    try {
        ...
        # The application repairs invalid settings using its defaults.
    }
    return $true
}
...
function Assert-ProjectChildPath {
    ...
    if ($target -eq $root -or -not $target.StartsWith(
        ...
        throw "Refusing to modify a directory outside OmniSonic: $target"
    }
    return $target
}
...
function Get-ProjectFingerprint {
    $parts = foreach ($relativePath in @(
        "pyproject.toml", "requirements-desktop.txt", "installer_backends.json"
    )) {
...
function Resolve-RequestedBackend {
    ...
    if (-not [string]::IsNullOrWhiteSpace($EnvironmentValue)) {
        ...
        if ($environmentBackend -notin $allowed) {
            throw "OMNISONIC_BACKEND must be one of: auto, cuda, rocm, xpu, cpu."
        }
...
function Select-FirstRunMode {
    ...
    if ($SystemPython) {
        ...
        Write-Host "[1] Portable Python inside OmniSonic (recommended)"
        Write-Host "    Wlasny Python wewnatrz OmniSonic (zalecane)"
    ...
    Write-Host "[1] Download portable Python $PythonVersion for OmniSonic (recommended)"
    Write-Host "    Pobierz wlasnego Pythona $PythonVersion dla OmniSonic (zalecane)"
...
function Resolve-PythonMode {
    ...
    if ($preferred -eq "Auto" -and $env:OMNISONIC_PYTHON_MODE -in @("System", "Portable")) {
        $preferred = $env:OMNISONIC_PYTHON_MODE
    }
    ...
    if ($preferred -eq "System" -and -not $systemPython) {
        ...
        if ($WasExplicit -or $env:OMNISONIC_PYTHON_MODE -eq "System") {
...
function Invoke-AcceleratorProbe {
    ...
    $output = @(& $Python -m omnisonic.accelerator --validate $ExpectedBackend --json 2>&1)
...
function Read-ReadyMarker {
    ...
    $path = Join-Path $EnvironmentRoot ".omnisonic-ready"
...
function Test-Runtime {
    ...
    try {
        ...
        & $Python -c "import accelerate, librosa, numpy, sounddevice, soundfile, torch, torchaudio, transformers, webdataset, wx; import omnivoice, omnisonic.app" 2>$null
        if ($LASTEXITCODE -ne 0) { throw "One or more OmniSonic runtime imports failed." }
...
function Write-ReadyMarker {
    ...
    $marker = Join-Path $EnvironmentRoot ".omnisonic-ready"
...
function Install-BackendRuntime {
    ...
    ) -Description "Installing OmniSonic desktop dependencies"
    Invoke-Checked -FilePath $Python -Arguments @(
        "-m", "pip", "install", "--no-deps", "--no-build-isolation",
        "--no-warn-script-location", "-e", $ProjectRoot
    ) -Description "Installing OmniSonic without replacing the selected PyTorch build"
    Invoke-Checked -FilePath $Python -Arguments @(
        "-m", "pip", "check"
    ) -Description "Checking installed dependencies"
    Invoke-Checked -FilePath $Python -Arguments @(
        "-c", "import accelerate, librosa, numpy, sounddevice, soundfile, torch, torchaudio, transformers, webdataset, wx; import omnivoice, omnisonic.app"
    ) -Description "Validating the complete OmniSonic desktop runtime"
...
function Show-HardwareSummary {
    ...
    if ($SelectedBackend -eq "cpu") {
        Write-WarningMessage "No supported GPU accelerator was selected. OmniSonic will use CPU and generation can be substantially slower."
        Write-Host "Nie wybrano obslugiwanego akceleratora GPU. OmniSonic uzyje CPU, wiec generowanie moze byc znacznie wolniejsze."
    }
    else {
...
function Invoke-SelfTest {
    ...
    foreach ($requiredPath in @(
        "pyproject.toml", "requirements-desktop.txt", "installer_backends.json",
        "omnisonic\app.py", "omnisonic\accelerator.py", "start_desktop.bat"
    )) {
    ...
    if ($runtimeRequirements | Where-Object { $_ -match '^\s*torch(audio)?\s*[=<>]' }) {
        throw "requirements-desktop.txt must not install torch or torchaudio."
    }
    ...
    Write-Host "OmniSonic launcher self-test: OK (CUDA, ROCm, XPU, CPU)"
}

function Invoke-Main {
    Set-Location -LiteralPath $ProjectRoot
    if ($QueryHiddenLaunch) {
        if (Test-HiddenLaunchReady) { Write-Output "HIDE" }
