...
function Remove-LauncherDirectory {
    ...
    if (-not $target.StartsWith($root, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing to remove a directory outside OmniSonic: $target"
    }
...
function Test-Runtime {
    ...
    try {
        & $Python -c "import accelerate, librosa, numpy, sounddevice, soundfile, torch, torchaudio, transformers, webdataset, wx; import omnivoice, omnisonic.app; assert torch.__version__" 2>$null
        if ($LASTEXITCODE -ne 0) {
            return $false
        }
...
function Install-Or-RepairRuntime {
    ...
    Invoke-Checked $Python @(
        ...
        "-e", ".[desktop]"
    ) "Installing OmniSonic, wxPython, and audio support"
    Invoke-Checked $Python @("-m", "pip", "check") "Checking installed dependencies"
    Invoke-Checked $Python @(
        "-c",
        "import accelerate, librosa, numpy, sounddevice, soundfile, torch, torchaudio, transformers, webdataset, wx; import omnivoice, omnisonic.app"
    ) "Validating the complete OmniSonic desktop runtime"
...
function Select-FirstRunMode {
    ...
    if ($env:OMNISONIC_PYTHON_MODE -in @("System", "Portable")) {
        return $env:OMNISONIC_PYTHON_MODE
    }
    ...
    Write-Host "[1] Download portable Python for OmniSonic (recommended)"
    Write-Host "    Pobierz wlasnego Pythona dla OmniSonic (zalecane)"
...
function Invoke-SelfTest {
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $ProjectRoot "omnisonic\app.py"))) {
        throw "The OmniSonic application package is missing."
    }
